...
export async function PATCH(
  ...
  try {
    ...
    const sql = `UPDATE job_applications SET ${sets.join(', ')} WHERE id = ${Number(params.id)} RETURNING *`
...
export async function DELETE(
  ...
  try {
    const records = await query(`SELECT resume_url FROM job_applications WHERE id = ${Number(params.id)}`)
    ...
    const sql = `DELETE FROM job_applications WHERE id = ${Number(params.id)} RETURNING *`
    const data = await query(sql)
    return Response.json(data)
  } catch (e) {
    return Response.json({ error: (e as Error).message }, { status: 500 })
